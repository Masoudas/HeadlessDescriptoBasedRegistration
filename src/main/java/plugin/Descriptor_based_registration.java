...
public class Descriptor_based_registration implements PlugIn {
	...
	public void run(String arg0) {
		...
		// gd.addMessage( "Warning: if images are of RGB or 8-bit color they will be
		// converted to hyperstacks.");
		// gd.addMessage( "Please note that the SPIM Registration is based on a
		// publication.\n" +
		// "If you use it successfully for your research please be so kind to cite our
		// work:\n" +
		...
		File baseFile = new File(
				"/home/masoud/Downloads/Descriptor_based_registration-master/src/test/resources/plugin/TwoProperBeadImages",
				"Beads_test2_C1.tif");

		ImagePlus imp1 = ImageJFunctions
				.showUnsignedShort(opener.openImgs(baseFile.getAbsolutePath(), new UnsignedShortType()).get(0), "registered");

		File toRegisterFile = new File(
				"/home/masoud/Downloads/Descriptor_based_registration-master/src/test/resources/plugin/TwoProperBeadImages",
				"Beads_test2_C2.tif");
		ImagePlus imp2 = ImageJFunctions.showUnsignedShort(
				opener.openImgs(toRegisterFile.getAbsolutePath(), new UnsignedShortType()).get(0), "base");

		// ImagePlus imp1 = WindowManager.getImage(idList[defaultImg1 =
		// gd.getNextChoiceIndex()]);
		// ImagePlus imp2 = WindowManager.getImage(idList[defaultImg2 =
		// gd.getNextChoiceIndex()]);
		...
		if (reApply) {
			if (dimensionality < lastDimensionality) {
				IJ.log("Cannot reapply, cannot apply a " + lastModel1.getClass().getSimpleName() + " to "
						+ dimensionality + " data.");
				defaultReApply = false;
				return;
			} else if (dimensionality > lastDimensionality) {
				IJ.log("WARNING: applying a " + lastModel1.getClass().getSimpleName() + " to " + dimensionality
						+ " data.");
			}
	...
	public String[] transformationModels2d = new String[] { "Translation (2d)", "Rigid (2d)", "Similarity (2d)",
			"Affine (2d)", "Homography (2d)" };
	public String[] transformationModels3d = new String[] { "Translation (3d)", "Rigid (3d)", "Similarity (3d)",
			"Affine (3d)" };
	...
	public static String[] detectionBrightness = { "Very low", "Low", "Medium", "Strong", "Advanced ...",
			"Interactive ..." };
	...
	public static String[] detectionSize = { "2 px", "3 px", "4 px", "5 px", "6 px", "7 px", "8 px", "9 px", "10 px",
	...
	public static String[] orientation = { "Not prealigned", "Approxmiately aligned",
	...
	protected DescriptorParameters getParameters(final ImagePlus imp1, final ImagePlus imp2, final int dimensionality) {
		...
		gd.addChoice("Subpixel_Localization", Descriptor_based_series_registration.localizationChoice,
				Descriptor_based_series_registration.localizationChoice[Descriptor_based_series_registration.defaultLocalization]);
		...
		if (regularize) {
			...
			gdRegularize.addChoice("Transformation_model", transformationModel,
			...
			if (dimensionality == 2) {
				switch (modelIndex) {
					case 0:
						params.model = new InterpolatedAffineModel2D(params.model, new TranslationModel2D(),
								(float) params.lambda);
						break;
					case 1:
						params.model = new InterpolatedAffineModel2D(params.model, new RigidModel2D(),
								(float) params.lambda);
						break;
					case 2:
						params.model = new InterpolatedAffineModel2D(params.model, new SimilarityModel2D(),
								(float) params.lambda);
						break;
					case 3:
						params.model = new InterpolatedAffineModel2D(params.model, new AffineModel2D(),
								(float) params.lambda);
						break;
					case 4:
				...
				switch (modelIndex) {
					case 0:
						params.model = new InterpolatedAffineModel3D(params.model, new TranslationModel3D(),
								(float) params.lambda);
						break;
					case 1:
						params.model = new InterpolatedAffineModel3D(params.model, new RigidModel3D(),
								(float) params.lambda);
						break;
					case 2:
						params.model = new InterpolatedAffineModel3D(params.model, new SimilarityModel3D(),
								(float) params.lambda);
						break;
					case 3:
						params.model = new InterpolatedAffineModel3D(params.model, new AffineModel3D(),
								(float) params.lambda);
						break;
					default:
						params.model = new InterpolatedAffineModel3D(params.model, new RigidModel3D(),
		...
		// one of them is by default interactive, then all are interactive
		if (detectionBrightnessIndex == detectionBrightness.length - 1 || detectionSizeIndex == detectionSize.length - 1
		...
		if (similarOrientation == 2) {
			if (TranslationModel3D.class.isInstance(params.model)
				...
				gd2.addStringField("Affine_matrix", m00 + ", " + m01 + ", " + m02 + ", " + m03 + ", " + m10 + ", " + m11
						+ ", " + m12 + ", " + m13 + ", " + m20 + ", " + m21 + ", " + m22 + ", " + m23, 80);
				...
				String entry = Apply_External_Transformation.removeSequences(gd2.getNextString().trim(),
						new String[] { "(", ")", "{", "}", "[", "]", "<", ">", ":", "m00", "m01", "m02", "m03", "m10",
				...
				model.set((float) m00, (float) m01, (float) m02, (float) m03, (float) m10, (float) m11, (float) m12,
				...
				gd2.addStringField("Affine_matrix",
						m00 + ", " + m01 + ", " + m02 + ", " + m10 + ", " + m11 + ", " + m12, 60);
				...
				gd2.addStringField("Homography_matrix",
						m00 + ", " + m01 + ", " + m02 + ", " + m10 + ", " + m11 + ", " + m12, 70);
				...
				model.set((float) m00, (float) m01, (float) m02, (float) m10, (float) m11, (float) m12, (float) m20,
						(float) m21, (float) m22);
				params.initialModel = model;
				...
				model.set((float) (defaultScale * Math.cos(Math.toRadians(defaultDegrees1))),
						(float) (defaultScale * Math.sin(Math.toRadians(defaultDegrees1))), 0, 0);
				params.initialModel = model;
			} else {
	 ...
	 * 
	 * The results are stored in the same array. If called with values[ 2 ], sigma2
	 ...
	 * @return {@link InteractiveDoG} - the instance for querying additional
	 *         parameters
	 */
	public static InteractiveDoG getInteractiveDoGParameters(final ImagePlus imp, final int channel,
			final double values[], final float sigmaMax) {
	...
	public static void main(String[] args) {
		new Descriptor_based_registration().run("arg0");
	}
}
